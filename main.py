...
llm = llm_gemini.with_fallbacks([llm_groq])  # Tenta Gemini primeiro, depois Groq se necessário
...
O dia e hora do momento em que o usuário mandou a mensagem são: {agora}
"""
...
while True:
  ...
  try:
    resposta = app.invoke(
      {"messages": [{"role" : "human", "content": user_input}]}, config={"configurable": {"thread_id": "meu_id_de_sessao"}},
    )
