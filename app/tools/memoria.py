...
Migração de tool_mongodb.py (raiz) para dentro de app/tools/, com três correções:
o import passou a apontar para app.memory, o arquivo entrou na convenção do
projeto e a leitura do config deixou de estourar KeyError quando ele não chega.

O nome é "memoria" e não "tools_mongodb" porque a convenção aqui nomeia tools
pelo domínio, não pela tecnologia — repare que financeiro.py não se chama
tools_postgres.py. Se amanhã a memória migrar de banco, o nome continua certo e
nenhum agente precisa saber da troca.
"""
...
def buscar_historico(busca: str, config: RunnableConfig) -> str:
    ...
    já existem as tools específicas do agente financeiro (search_transactions,
